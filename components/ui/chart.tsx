...
export function AreaChartComponent({ data, config, className }: AreaChartProps) {
  const { Area, AreaChart, CartesianGrid, ResponsiveContainer, Tooltip, XAxis, YAxis } = require("recharts");
  const keys = Object.keys(config);
  return (
    <div className={className}>
      <ResponsiveContainer width="100%" height={300}>
        <AreaChart data={data} margin={{ top: 16, right: 16, bottom: 0, left: 0 }}>
          <defs>
            {keys.map((key) => (
              <linearGradient key={key} id={`fill-${key}`} x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor={config[key].color} stopOpacity={0.15} />
          ...
          <Tooltip cursor={{ fill: "#F0EDF9" }} contentStyle={{ borderRadius: "8px", border: "1px solid #E5E5E5", fontFamily: "Inter", fontSize: "13px" }} />
          {keys.map((key) => (
            <Area key={key} dataKey={key} fill={`url(#fill-${key})`} stroke={config[key].color} strokeWidth={2} />
          ))}
        </AreaChart>
      </ResponsiveContainer>
    </div>
