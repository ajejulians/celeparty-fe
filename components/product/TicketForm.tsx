    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      ...
      <div className="bg-white p-6 rounded-xl border border-neutral-200 space-y-6">
        <div className="flex items-center gap-2 border-b border-neutral-100 pb-3">
          ...
          <h2 className="font-quick font-bold text-lg text-neutral-900">Informasi Event & Tiket</h2>
        </div>
        
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1">
            <Label className="font-sans text-neutral-700 mb-2 block">Poster Event</Label>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="space-y-2">
              ...
              <Input {...register("name")} placeholder="Contoh: Java Jazz Festival 2026" className="font-sans text-base h-11" />
              {errors.name && <p className="text-xs text-c-red font-sans">{errors.name.message}</p>}
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label className="font-sans text-neutral-700">Tanggal Pelaksanaan</Label>
                <Input type="date" {...register("eventDate")} className="font-sans text-base h-11" />
                {errors.eventDate && <p className="text-xs text-c-red font-sans">{errors.eventDate.message}</p>}
              </div>
              <div className="space-y-2">
                <Label className="font-sans text-neutral-700">Lokasi / Venue</Label>
                <Input {...register("location")} placeholder="Contoh: JIExpo Kemayoran" className="font-sans text-base h-11" />
                {errors.location && <p className="text-xs text-c-red font-sans">{errors.location.message}</p>}
              </div>
            </div>

            <div className="space-y-2">
              <Label className="font-sans text-neutral-700">Deskripsi Event</Label>
              <textarea {...register("description")} rows={4} className="w-full px-3 py-2.5 border border-neutral-200 rounded-lg bg-white font-sans text-base focus:outline-none focus:border-c-blue resize-none" placeholder="Deskripsikan acara Anda..." />
      ...
      <div className="bg-white p-6 rounded-xl border border-neutral-200 space-y-6">
        <div className="flex items-center justify-between border-b border-neutral-100 pb-3">
          <h2 className="font-quick font-bold text-lg text-neutral-900">Kategori Tiket & Harga</h2>
          <Button type="button" onClick={() => append({ name: "", price: 0, quota: 100 })} variant="outline" size="sm" className="font-quick text-xs gap-1 h-8">
          ...
          {fields.map((field, index) => (
            <div key={field.id} className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-end bg-neutral-50 p-4 rounded-lg border border-neutral-100">
              <div className="sm:col-span-5 space-y-1">
                <Label className="font-sans text-xs text-neutral-500">Nama Kategori (Varian)</Label>
                <Input {...register(`variants.${index}.name`)} placeholder="Contoh: Early Bird VIP" className="font-sans text-base h-10 bg-white" />
              ...
              <div className="sm:col-span-2 space-y-1">
                <Label className="font-sans text-xs text-neutral-500">Kuota Tiket</Label>
                <Input type="number" {...register(`variants.${index}.quota`)} placeholder="100" className="font-sans text-base h-10 bg-white" />
