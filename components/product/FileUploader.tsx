  ...
  return (
    <div className="w-full">
      {preview ? (
        ...
        <label className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-neutral-300 rounded-lg cursor-pointer bg-neutral-50 hover:bg-neutral-100 transition-colors">
          <div className="flex flex-col items-center justify-center pt-5 pb-6">
            <UploadCloud className="w-10 h-10 text-neutral-400 mb-3" />
            {isCompressing ? (
              <p className="font-sans text-sm text-c-blue font-semibold">{progressText}</p>
            ) : (
              <>
                <p className="mb-2 text-sm text-neutral-500 font-sans">
                  <span className="font-semibold text-c-blue">Klik untuk unggah</span> atau seret dan lepas
                </p>
                <p className="text-xs text-neutral-400 font-sans">PNG, JPG, JPEG (Maks. 5MB, Auto Compress)</p>
              </>
            )}
          </div>
          <input
