...
function NavLink({ item, activePath, exact }: { item: NavItem; activePath: string | undefined; exact?: boolean }) {
  const isActive = exact
    ? activePath === item.href
    : activePath === item.href || (item.href !== "#" && !!activePath && activePath.startsWith(item.href + "/"));
  ...
  return (
    ...
    >
      <span className={cn("w-5 h-5 shrink-0", isActive ? "text-c-blue" : "text-neutral-400 group-hover:text-neutral-600")}>
        {item.icon}
      </span>
...
function NavGroup({ item, activePath }: { item: NavItem; activePath: string | undefined }) {
  ...
  return (
    <div>
      ...
      >
        <span className={cn("w-5 h-5 shrink-0", isGroupActive ? "text-c-blue" : "text-neutral-400 group-hover:text-neutral-600")}>
          {item.icon}
        </span>
        <span className="flex-1">{item.label}</span>
      ...
      {open && item.children && (
        <div className="ml-4 mt-1 space-y-1 border-l border-neutral-200 pl-3">
          {item.children.map((child) => (
            <NavLink key={child.href} item={child} activePath={activePath} exact />
          ))}
        </div>
      )}
    </div>
