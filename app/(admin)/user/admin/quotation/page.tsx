...
import { Copy, CheckCircle2, Calculator, ExternalLink } from "lucide-react";
...
export default function AdminQuotationPage() {
  ...
  const onSubmit = async (data: QuotationFormOutput) => {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="font-sans text-sm text-neutral-500 mt-1">
            Buat penawaran khusus dan hasilkan tautan pembayaran untuk pelanggan.
          </p>
        </div>
        ...
        {!generatedLink ? (
          <Card>
            <CardHeader>
              <CardTitle className="font-quick text-lg text-c-blue">Form Penawaran Baru</CardTitle>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label className="font-sans text-neutral-700">Nama Pelanggan</Label>
                    <Input
                    ...
                    />
                    {errors.customerName && (
                      <p className="text-xs text-c-red font-sans">{errors.customerName.message}</p>
                    )}
                  </div>
                  
                  <div className="space-y-2">
                    <Label className="font-sans text-neutral-700">Email Pelanggan</Label>
                    <Input
                      type="email"
                      {...register("email")}
                    ...
                    />
                    {errors.email && (
                      <p className="text-xs text-c-red font-sans">{errors.email.message}</p>
                    )}
                  </div>

                  <div className="space-y-2">
                    <Label className="font-sans text-neutral-700">Nomor WhatsApp</Label>
                    <Input
                      type="tel"
                      {...register("wa")}
                    ...
                    />
                    {errors.wa && (
                      <p className="text-xs text-c-red font-sans">{errors.wa.message}</p>
                    )}
                  </div>

                  <div className="space-y-2">
                    <Label className="font-sans text-neutral-700">Nama Produk Spesifik</Label>
                    <Input
                    ...
                    />
                    {errors.productName && (
                      <p className="text-xs text-c-red font-sans">{errors.productName.message}</p>
                    )}
                  </div>

                  <div className="space-y-2 md:col-span-2">
                    <Label className="font-sans text-neutral-700">Harga Kesepakatan (Rp)</Label>
                    <Input
                      type="number"
                      {...register("price")}
                    ...
                    />
                    {errors.price && (
                      <p className="text-xs text-c-red font-sans">{errors.price.message}</p>
                    )}
                  </div>
                </div>
          ...
          <Card className="border-emerald-100 bg-emerald-50/30">
            ...
            <CardContent className="pt-4">
              ...
              <div className="mt-8 pt-6 border-t border-emerald-100 flex justify-center gap-4">
                <Button variant="outline" onClick={resetForm} className="font-quick font-semibold text-neutral-700 h-11">
                  Buat Penawaran Baru
                </Button>
                <a href={generatedLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-white border border-neutral-200 text-neutral-700 font-quick font-semibold text-sm px-6 rounded-md hover:bg-neutral-50 h-11 transition-colors">
