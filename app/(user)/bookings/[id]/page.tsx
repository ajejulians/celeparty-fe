...
export type BookingStatus = "Menunggu Pembayaran" | "Dibayar" | "Proses Vendor" | "Selesai" | "Dibatalkan";
...
const StatusTracker = ({ status }: { status: BookingStatus }) => {
  ...
  if (status === "Dibatalkan") {
    return (
      <div className="bg-c-red-50 border border-c-red/20 rounded-xl p-4 flex items-center gap-3">
        ...
        <div>
          <h4 className="font-quick font-bold text-c-red">Pesanan Dibatalkan</h4>
          <p className="font-sans text-sm text-c-red/80">Pesanan ini telah dibatalkan.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-2xl border border-neutral-100 p-6 shadow-sm">
      <h3 className="font-quick font-bold text-lg text-neutral-900 mb-6">Status Pesanan</h3>
      <div className="flex flex-col md:flex-row justify-between relative">
        ...
        {steps.map((step, idx) => {
          ...
          return (
            <div key={step} className="flex md:flex-col items-center gap-4 md:gap-3 mb-6 md:mb-0 z-10 relative">
              <div 
              ...
              <div className="md:text-center">
                <p className={`font-quick font-semibold text-sm ${isActive ? "text-c-blue" : isCompleted ? "text-neutral-900" : "text-neutral-400"}`}>
                  {step}
                </p>
              </div>
            </div>
...
const VendorAndProductSummary = ({ vendor, product }: { vendor: VendorDetail, product: ProductDetail }) => {
  return (
    <div className="bg-white rounded-2xl border border-neutral-100 shadow-sm overflow-hidden">
      <div className="p-4 bg-slate-50 border-b border-neutral-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-quick font-bold text-neutral-900 text-sm">{vendor.name}</h4>
      ...
      <div className="p-6 flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex-1">
          <h3 className="font-quick font-bold text-lg text-neutral-900 mb-1">{product.title}</h3>
          <p className="font-sans text-sm text-neutral-500 mb-3">Varian: <span className="font-medium text-neutral-700">{product.variant}</span></p>
...
const LogisticsCard = ({ logistics }: { logistics: LogisticsDetail }) => {
  return (
    <div className="bg-white rounded-2xl border border-neutral-100 p-6 shadow-sm">
      <h3 className="font-quick font-bold text-lg text-neutral-900 mb-6">Detail Logistik & Acara</h3>
      ...
      <div className="space-y-4">
        <div className="flex gap-3">
          ...
          <div>
            <p className="text-xs font-sans text-neutral-500 mb-0.5">Tanggal Acara</p>
            <p className="font-quick font-semibold text-neutral-900">{logistics.eventDate}</p>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <div>
            <p className="text-xs font-sans text-neutral-500 mb-0.5">Waktu Setup/Loading</p>
            <p className="font-quick font-semibold text-neutral-900">{logistics.setupTime}</p>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <div>
            <p className="text-xs font-sans text-neutral-500 mb-0.5">Alamat Venue</p>
            <p className="font-sans text-sm text-neutral-700 leading-relaxed">{logistics.venueAddress}</p>
          </div>
        </div>

        {logistics.specialNotes && (
          <div className="mt-6 p-4 rounded-xl bg-slate-50 border border-slate-100">
            <p className="text-xs font-sans text-neutral-500 mb-1">Catatan Tambahan:</p>
            <p className="font-sans text-sm text-neutral-700 italic">"{logistics.specialNotes}"</p>
          </div>
        )}
      </div>
    </div>
...
const EscrowSummary = ({ pricing, status }: { pricing: PriceBreakdown, status: BookingStatus }) => {
  const formatIDR = (num: number) => 
    new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(num);

  return (
    <div className="bg-white rounded-2xl border border-neutral-100 p-6 shadow-sm sticky top-24">
      <h3 className="font-quick font-bold text-lg text-neutral-900 mb-6">Ringkasan Pembayaran</h3>
      ...
      <div className="space-y-3 font-sans text-sm mb-6">
        ...
        <div className="pt-3 border-t border-dashed border-neutral-200 flex justify-between items-center">
          <span className="font-quick font-bold text-neutral-900">Total Pembayaran</span>
          <span className="font-quick font-bold text-xl text-c-blue">{formatIDR(pricing.grandTotal)}</span>
        </div>
      </div>

      <div className="flex items-center gap-2 justify-center py-3 bg-emerald-50 rounded-lg text-emerald-700 mb-6">
        <ShieldCheck className="w-5 h-5" />
        <span className="font-sans text-xs font-medium">Dana diamankan oleh Celeparty Escrow</span>
      </div>
...
export default function BookingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchBooking = async () => {
      ...
      if (order.paymentStatus === "cancelled" || order.vendorStatus === "rejected") {
        status = "Dibatalkan";
      } else if (order.paymentStatus === "settlement" && order.vendorStatus === "approved") {
        status = "Selesai";
      } else if (order.paymentStatus === "settlement" && order.vendorStatus === "pending") {
        status = "Dibayar";
      } else if (order.paymentStatus === "pending") {
        status = "Menunggu Pembayaran";
      }

      const vendorAvatar = "https://images.unsplash.com/photo-1516280440502-8610eb675005?w=128&h=128&fit=crop"; // Mock avatar
      const productImg = product?.imageUrl || "https://images.unsplash.com/photo-1598488035139-bdbb2231ce04?w=600&h=400&fit=crop";
      ...
      setBooking({
        ...
        logistics: {
          eventDate: new Date(order.eventDate).toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric' }),
  ...
  if (!booking) return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center flex-col">
      ...
      <Link href="/bookings" className="inline-flex items-center gap-2 text-c-blue font-semibold mt-4 bg-c-blue-50 px-6 py-2 rounded-full">
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
      <div className="max-w-6xl mx-auto px-4 pt-8">
        ...
        <Link href="/bookings" className="inline-flex items-center gap-2 text-neutral-500 hover:text-c-blue transition-colors font-sans text-sm mb-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <div>
            <span className={`px-4 py-1.5 rounded-full font-quick font-bold text-sm ${statusColors[booking.status]}`}>
              {booking.status}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <VendorAndProductSummary vendor={booking.vendor} product={booking.product} />
