...
import { Wallet, ArrowUpRight, ArrowDownLeft, Download, X } from "lucide-react";
...
export default function VendorWalletPage() {
  const [saldoActive, setSaldoActive] = useState(21500000);
  const [saldoRefund, setSaldoRefund] = useState(1500000);
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="font-quick font-bold text-2xl text-neutral-900">Wallet & Saldo</h1>
          <p className="font-sans text-sm text-neutral-500 mt-1">Kelola saldo dan riwayat transaksi Anda</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl border border-neutral-200 p-6 flex flex-col justify-between">
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-sans text-sm text-neutral-500">Saldo Aktif (Tersedia)</p>
                  <p className="font-quick font-bold text-3xl text-neutral-900">{formatCurrency(saldoActive)}</p>
                </div>
              </div>
            </div>
              ...
              <Dialog.Portal>
                ...
                <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-xl shadow-xl z-50 p-6 animate-slide-up focus:outline-none">
                  <div className="flex items-center justify-between mb-6">
                    <Dialog.Title className="font-quick font-bold text-xl text-neutral-900">Tarik Saldo</Dialog.Title>
                  ...
                  <form onSubmit={handleWithdraw} className="space-y-6">
                    <div className="space-y-2">
                      <Label className="font-sans text-neutral-700">Jumlah Penarikan (Rp)</Label>
                      <Input
                        type="number"
                        ...
                        max={saldoActive}
                      />
                      <p className="text-xs font-sans text-neutral-500">Maksimal penarikan: {formatCurrency(saldoActive)}</p>
                    </div>

                    <div className="space-y-2">
                      <Label className="font-sans text-neutral-700">Bank Tujuan</Label>
                    ...
                    <Button type="submit" disabled={isSubmitting || !withdrawAmount} className="w-full h-11 font-quick font-semibold bg-c-blue text-white hover:brightness-95 text-base">
          ...
          <div className="bg-white rounded-xl border border-neutral-200 p-6 flex flex-col justify-between">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <p className="font-sans text-sm text-neutral-500">Saldo Tertahan (Escrow/Refund)</p>
                <p className="font-quick font-bold text-3xl text-neutral-900">{formatCurrency(saldoRefund)}</p>
              </div>
            </div>
            <p className="font-sans text-sm text-neutral-500 leading-relaxed">
              Dana ini sedang ditahan dalam sistem Escrow menunggu konfirmasi pesanan selesai atau dalam proses pengembalian ke pelanggan.
            </p>
          </div>
        </div>
