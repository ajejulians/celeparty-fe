...
import {
	...
	createEventSchema,
} from "@/lib/ticket-validators";

const CKEditorComponent = dynamic(
	() => import("@/components/blog/CKEditorComponent"),
	{
		ssr: false,
		loading: () => (
			<div className="h-64 bg-neutral-100 rounded-lg animate-pulse flex items-center justify-center">
				<p className="font-sans text-sm text-neutral-400">Memuat editor...</p>
			</div>
		),
	},
);
				...
				<div className="flex items-center gap-4 mb-8">
					...
					<div>
						...
						<p className="text-sm text-neutral-500 font-sans mt-1">
							Lengkapi detail event dan definisikan kategori tiket
						</p>
					</div>
				</div>
				...
				<form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pb-20">
					...
					<div className="bg-white p-6 rounded-xl border border-neutral-200 shadow-sm space-y-6">
						<div className="flex items-center gap-2 border-b border-neutral-100 pb-3">
							...
							<h2 className="font-quick font-semibold text-xl text-neutral-900">
								Detail Utama
							</h2>
						</div>

						<div className="space-y-2">
							<Label className="text-sm font-sans text-neutral-500">
								Banner / Poster Event
							</Label>
							<FileUploader onFileChange={setBanner} />
							<p className="font-sans text-xs text-neutral-400">
								<Info className="w-3 h-3 inline mr-1" />
								Rasio 16:9, maks 5MB. Format PNG/JPG.
							</p>
						</div>

						<div className="space-y-2">
							<Label className="text-sm font-sans text-neutral-500">
								Nama Event <span className="text-c-red">*</span>
							</Label>
							<Input
								{...register("eventName")}
							...
							/>
							{errors.eventName && (
								<p className="text-xs text-c-red font-sans mt-1">
						...
						<div className="space-y-2">
							<Label className="text-sm font-sans text-neutral-500">
								Kategori Event <span className="text-c-red">*</span>
							</Label>
							<Controller
								name="eventCategory"
								control={control}
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							<div className="space-y-2">
								<Label className="text-sm font-sans text-neutral-500">
									Waktu Mulai <span className="text-c-red">*</span>
								</Label>
								<div className="relative">
									...
									<Input
										type="datetime-local"
										{...register("dateStart")}
										className="font-sans text-base h-11 pl-10"
									/>
								</div>
								{errors.dateStart && (
									<p className="text-xs text-c-red font-sans mt-1">
							...
							<div className="space-y-2">
								<Label className="text-sm font-sans text-neutral-500">
									Waktu Selesai <span className="text-c-red">*</span>
								</Label>
								<div className="relative">
									<CalendarDays className="w-4 h-4 absolute left-3 top-3.5 text-neutral-400 pointer-events-none" />
									<Input
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							<div className="space-y-2">
								<Label className="text-sm font-sans text-neutral-500">
									Lokasi / Venue <span className="text-c-red">*</span>
								</Label>
								<div className="relative">
					...
					<div className="bg-white p-6 rounded-xl border border-neutral-200 shadow-sm space-y-6">
						...
						<div className="space-y-6">
							<div className="space-y-2">
								<Label className="text-sm font-sans text-neutral-500">
									Deskripsi Event <span className="text-c-red">*</span>
								</Label>
								<Controller
									name="description"
									control={control}
							...
							<div className="space-y-2">
								<Label className="text-sm font-sans text-neutral-500">
									Syarat & Ketentuan <span className="text-c-red">*</span>
								</Label>
								<Textarea
									{...register("terms")}
									rows={5}
						...
						<div className="space-y-4">
							{fields.map((field, index) => {
								...
								return (
									<div
										key={field.id}
										className="border border-neutral-200 rounded-lg p-4 sm:p-5 bg-neutral-50"
									>
										<div className="flex items-center justify-between mb-4">
											<div className="flex items-center gap-2">
												...
												<span className="font-quick font-semibold text-sm text-neutral-700">
													...
													{isFree && (
														<span className="ml-2 text-xs text-c-green font-bold font-sans">
															GRATIS
														</span>
													)}
												</span>
										...
										<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
											<div className="space-y-2">
												<Label className="text-sm font-sans text-neutral-500">
													Nama Tiket <span className="text-c-red">*</span>
												</Label>
												<Input
													{...register(`categories.${index}.name`)}
													placeholder="Contoh: VIP, Reguler, Early Bird"
													className="font-sans h-10 bg-white"
												/>
											...
											<div className="space-y-2">
												<Label className="text-sm font-sans text-neutral-500">
													Harga (Rp) <span className="text-c-red">*</span>
												</Label>
												<Input
													type="number"
													min={0}
											...
											<div className="space-y-2">
												<Label className="text-sm font-sans text-neutral-500">
													Kuota <span className="text-c-red">*</span>
												</Label>
												<Input
													type="number"
													min={1}
													{...register(`categories.${index}.quota`)}
													placeholder="100"
													className="font-sans h-10 bg-white"
												/>
												{errors.categories?.[index]?.quota && (
													<p className="text-[10px] text-c-red font-sans">
														{errors.categories[index]?.quota?.message}
													</p>
												)}
											</div>
											<div className="space-y-2">
												<Label className="text-sm font-sans text-neutral-500">
													Mulai Penjualan <span className="text-c-red">*</span>
												</Label>
												<Input
													type="date"
													{...register(`categories.${index}.saleStart`)}
													className="font-sans h-10 bg-white"
												/>
												{errors.categories?.[index]?.saleStart && (
													<p className="text-[10px] text-c-red font-sans">
														{errors.categories[index]?.saleStart?.message}
													</p>
												)}
											</div>
											<div className="space-y-2">
												<Label className="text-sm font-sans text-neutral-500">
													Akhir Penjualan <span className="text-c-red">*</span>
												</Label>
												<Input
													type="date"
													{...register(`categories.${index}.saleEnd`)}
													className="font-sans h-10 bg-white"
												/>
												{errors.categories?.[index]?.saleEnd && (
													<p className="text-[10px] text-c-red font-sans">
														{errors.categories[index]?.saleEnd?.message}
													</p>
												)}
											</div>
											<div className="sm:col-span-2 space-y-2">
												...
												/>
												{errors.categories?.[index]?.benefits && (
													<p className="text-[10px] text-c-red font-sans">
														{errors.categories[index]?.benefits?.message}
													</p>
												)}
											</div>
										</div>
					...
					<div className="fixed bottom-0 left-0 lg:left-64 right-0 z-10 bg-white border-t border-neutral-200 shadow-md p-4 flex justify-end gap-3">
						<Link href="/user/vendor/tickets">
							<Button
								type="button"
								variant="outline"
								className="font-quick font-semibold h-11 px-6"
							>
								Batal
							</Button>
						</Link>
						<Button
							...
							className="font-quick font-semibold h-11 px-8 bg-c-blue text-white hover:brightness-95 gap-2"
						>
