...
export default function MonitoringPage() {
	...
	const vendorOrders = mockRentals.filter(
	...
	const stats = useMemo(() => {
		...
		const overdue = vendorOrders.filter((o) => {
			...
			return (
				...
				o.paymentStatus !== "dp_refunded"
			);
		});

		return {
			totalActive: vendorOrders.filter(
				(o) =>
					o.paymentStatus !== "cancelled" &&
					o.paymentStatus !== "failed" &&
					o.paymentStatus !== "expired" &&
			...
			<div className="px-4 sm:px-6 lg:px-8 py-8">
				<div className="mb-8">
					...
					<p className="font-sans text-sm text-neutral-500 mt-1">
						Pantau barang yang sedang disewa
					</p>
				</div>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
					<Card>
						<CardHeader className="flex flex-row items-center justify-between pb-2">
							<CardTitle className="text-xs font-sans font-medium text-neutral-500 uppercase tracking-wide">
								Transaksi Aktif
							</CardTitle>
					...
					<Card>
						...
						<CardContent>
							<p
								className={`font-quick font-bold text-2xl ${stats.overdueCount > 0 ? "text-c-red" : "text-neutral-900"}`}
							>
								{stats.overdueCount}
							</p>
						</CardContent>
