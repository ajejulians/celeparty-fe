...
const _indonesianPhoneRegex = /^(\+62|62|08)\d{8,12}$/;
...
export const createEventSchema = z
	.object({
		...
		categories: z
			...
			.refine(
				(cats) =>
					new Set(cats.map((c) => c.name.trim().toLowerCase())).size ===
					cats.length,
	...
	.refine(
		...
		{
			message: "Tanggal akhir event tidak boleh sebelum tanggal mulai",
			path: ["dateEnd"],
		},
	);
